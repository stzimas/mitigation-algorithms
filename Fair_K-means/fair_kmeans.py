...
class KMeansBalanced:
    def __init__(self, n_clusters=3, max_iter=400, random_state=None, lambda_=0.5):
    ...
    def compute_forces(self, X, centroids):
        ...
        sign = np.where(self.attributes[:, None] == majority_adj, -1.0, 1.0)
        sign[majority_adj == -2] = 0

        new_metric = ((1 - self.lambda_) + self.lambda_ * imbalance * sign) * distances
        return new_metric
